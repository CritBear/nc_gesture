...
class Viewer:
    ...
    def load_file(self, path, pkl_idx=0, model_offset=vector(0, 0, 0), inference_path=None, is_original_data = True):
        if 'bvh' in path:
            with open(path) as f:
                bvh = Bvh(f.read())
...
def main():
    ...
    for data_idx in range(0, 100):
        ...
        viewer.load_file("data/decord_result_BaseMST_motion_body_fixed_nohand_all_9000.pkl", pkl_idx=data_idx, model_offset=vector(0, 0, 0), is_original_data = False)
        viewer.load_file("data/decord_result_BaseMST_motion_body_fixed_nohand_all_9000_2.pkl", pkl_idx=data_idx, model_offset=vector(150, 0, 0), is_original_data = False)
